...
const Info = (props: Props) => {
   ...
   const handleClickCreate = async (event: any) => {
      ...
      const customer_info = {
         mA_KH: "KH" + random_num,
         teN_KH: fullnameRef.current?.value,
         cmnd: cmndRef.current?.value,
         diA_CHI: addressRef.current?.value,
         email: emailRef.current?.value,
         Fax: faxRef.current?.value,
         sdt: phoneRef.current?.value,
         // LoaiPhong: typeRoomRef.current?.value,
         sO_DEM_LUU_TRU: numberNightRef.current?.value,
         yeU_CAU_DB: requireRef?.current.value,
      };
   ...
   const handleClick = async (event: any) => {
      ...
      const book = {
         ...
         loaiphong: typeRoomRef.current?.value,
         ngaY_TRA_PHONG: null,
         sO_DEM_LUU_TRU: numberNightRef.current?.value || 0,
         mA_GOIDV: comboC != "" ? comboC : null,
      };
      ...
      try {
         ...
         if (data1.data[0].MA_KH) {
            ...
            notify(`Add ${customer_info.mA_KH} Successfully`);
         }
      } catch (e) {}
   };
